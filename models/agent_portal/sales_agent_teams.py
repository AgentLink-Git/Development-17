...
class SalesAgentTeamWizard(models.TransientModel):
        ...
        for rec in self:
            if rec.target_type == "deal" and not rec.deal_id:
                ...
                raise ValidationError(
                    _("Please select a Listing to apply the team to.")
                )
    ...
    def apply_team(self):
        ...
        if not target_record:
            raise ValidationError(
                _("Please select a %s to apply the team to.") % target_type_name
            )

        # Check existing percentage for the end
        domain = [
        ...
        # Calculate total percentage to add from the team
        ...
        if existing_percentage + total_percentage_to_add > 100:
            raise ValidationError(
                _(
                    "Applying this team will exceed the total commission percentage for the selected %s and end."
                )
                % target_type_name
            )

        # Prepare referrals to create
        referrals = []
        ...
        # Create SalesAgentsAndReferrals records
    ...
    def _check_team_members(self):
        for team in self:
            if not team.team_member_ids:
                raise ValidationError(
                    _("The team '%s' must have at least one team member.") % team.name
                )
...
class SalesAgentTeamMember(models.Model):
    ...
    _sql_constraints = [
        (
            ...
            "A Sales Agent cannot be added multiple times to the same team.",
        ),
    ]
