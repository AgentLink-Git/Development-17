...
class DocumentType(models.Model):
    ...
    is_conveyancing_document = fields.Boolean(
        string="Is Conveyancing Document", default=False
    )

    convey_to_seller_law_firm = fields.Boolean(
        string="Convey to Seller Law Firm", default=False
    )
    convey_to_buyer_law_firm = fields.Boolean(
        string="Convey to Buyer Law Firm", default=False
    )
    convey_to_other_broker = fields.Boolean(
        string="Convey to Other Broker", default=False
    )
        ...
        for rec in self:
            if not rec.is_listing_document and not rec.is_deal_document:
                raise ValidationError(
                    _(
                        "A Document Type must be associated with at least a Listing or a Deal."
                    )
                )
        ...
        for rec in self:
            ...
            if rec.deal_id:
                ...
                required_status_codes = (
                    document_type.required_for_deal_status_ids.mapped("code")
                )

                if deal_status == "pending" and "pending" in required_status_codes:
                    rec.document_required = True
                elif (
                    deal_status == "collapsed" and "collapsed" in required_status_codes
                ):
                    rec.document_required = True
                    ...
                    if "removal_of_conditions" in required_status_codes:
                        ...
                        if rec.deal_id.condition_line_ids and all(
                            rec.deal_id.condition_line_ids.mapped("condition_removed")
                        ):
                            rec.document_required = True
            elif rec.listing_id:
                listing_status = rec.listing_id.status
                required_status_codes = (
                    document_type.required_for_listing_status_ids.mapped("code")
                )
