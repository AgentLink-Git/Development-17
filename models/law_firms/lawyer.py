...
class Lawyer(models.Model):
        ...
        for rec in self:
            if self._is_duplicate_lawyer(rec):
                raise ValidationError(
                    _(
                        'The contact "%s" is already a lawyer in the "%s" law firm. Please review existing entries before adding a new one.'
                    )
    ...
    def _update_partner_to_lawyer(self):
        ...
        _logger.info(
            "Updated partner %s to lawyer under law firm %s",
            self.partner_id.name,
            self.law_firm_id.partner_id.name,
        )
    ...
    def _get_partner_ids_by_name(self, name, operator):
        ...
        return (
            ...
            .search(["|", ("name", operator, name), ("parent_id.name", operator, name)])
            .ids
        )
